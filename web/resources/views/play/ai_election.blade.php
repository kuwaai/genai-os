                    ...
                    <section class="flex flex-col h-full">
                        ...
                        <div class="flex-1 border-gray-700 rounded-200 border-2 hidden" id="Game">
                            <div class="m-auto flex h-full flex-col">
                                <p class="mx-auto"><span class="text-red-500">10</span> days left before the election
                                    begin</p>
                                <span class="mx-auto">You can select different LLM and modify the prompt to affact the
                                    outcome speech</span>
                                <div class="flex-1 flex mb-2">
                                    ...
                                    <div class="flex-1 flex-col flex border-gray-700 rounded-lg border-2">
                                                ...
                                                @foreach (App\Models\LLMs::where('enabled', true)->orderby('order')->orderby('created_at')->get() as $LLM)
                                                    <li>
                                                        <button id="{{ 'llm_id_' . $LLM->id }}" type="button"
                                                            onclick='$("#llm_select").html($("#{{ 'llm_id_' . $LLM->id }}").html())'
                                        ...
                                        <textarea class="flex-1 resize-none bg-transparent" placeholder="Prompt here"></textarea>
                                        <textarea class="flex-1 resize-none bg-transparent mb-2" placeholder="Here's what the LLM gonna say in the speech..."
                                            readonly></textarea>
                                        <div class="mx-auto mb-2">
                                            <button
                                                class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded-lg text-center">Preview</button>
                                            <button
                                                class="ml-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg text-center">Send</button>
                                        </div>
                                    </div>
                                    <div class="flex-1 flex-col"></div>
                                </div>
