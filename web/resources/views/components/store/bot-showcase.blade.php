...
<div class="flex h-[148px]">
    ...
    <div class="flex overflow-hidden space-x-2 w-full bot-showcase"
        ...
        @foreach ($bots as $bot)
            <div onclick="detail_update({{json_encode(array_merge($bot->toArray(), ['image' => strpos($bot->image, 'data:image/png;base64') === 0 ? $bot->image : asset(Storage::url($bot->image))]))}})" data-modal-target="detail-modal" data-modal-toggle="detail-modal"
                ...
                <img id="llm_img" class="rounded-full mx-auto bg-black" width="50px" height="50px"
                    src="{{ strpos($bot->image, 'data:image/png;base64') === 0 ? $bot->image : asset(Storage::url($bot->image)) }}">
