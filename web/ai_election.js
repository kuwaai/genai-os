...
async function api_send(token, msg, llm_id){
	$url = `http://localhost/api_auth?key=${encodeURIComponent(process.env.APP_KEY)}&api_token=${token}&msg=${msg}&llm_id=${llm_id}`
	console.log($url)
...
io.on('connection', client => {
	...
	client.on("auth", (token) => {
		...
		fetch(`http://localhost/api_auth?key=${encodeURIComponent(process.env.APP_KEY)}&api_token=${token}`)
			.then((response) => {
				if (!response.ok) {
					client.disconnect();
					throw new Error('Can\'t reach the auth server!');
