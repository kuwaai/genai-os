...
class RequestChat implements ShouldQueue
{
    ...
    {
        Log::channel("analyze")->Debug("accessCode" . $access_code . "userID" . $this->user_id . "historyID " . $this->history_id . " input " . trim($this->input) . " length " . strlen(trim($this->input)));
        ...
        try {
            ...
            if ($response->getBody()->getContents() == 'BUSY') {
                ...
                try {
                    ...
                    Log::channel("analyze")->Debug("accessCode" . $access_code . "userID" . $this->user_id . "historyID " . $this->history_id . " output " . trim(str_replace("\n", "[NEWLINEPLACEHOLDERUWU]", $tmp)) . " took " . $elapsed . " generated " . strlen(trim($tmp)));
                }
            }
        } catch (Exception $e) {
        }
