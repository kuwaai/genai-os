<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        @else
            <!-- Accounnt -->
            <x-input-label for="email" :value="__('login.label.account')" />
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox"
                    ...
                    name="remember">
                <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ __('login.label.remember_me') }}</span>
            </label>
        </div>

        <div class="flex items-center justify-end mt-4">
            @if (App\Models\SystemSetting::smtpConfigured())
                <a class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800"
                    href="{{ route('password.request') }}">
                    {{ __('login.button.forgot_password') }}
                </a>
            @endif
