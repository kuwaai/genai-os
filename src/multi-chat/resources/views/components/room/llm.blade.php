...
@foreach ($DCs as $DC)
    @if (array_diff(explode(',', $DC->first()->identifier), $result->pluck('id')->toArray()) == [])
        <div class="mb-2 border border-black dark:border-white border-1 rounded-lg">
        ...
        })->select('llms.*', 'bots.*', 'chats.*', DB::raw('COALESCE(bots.description, llms.description) as description'), DB::raw('COALESCE(bots.config, llms.config) as config'), DB::raw('COALESCE(bots.image, llms.image) as image'))->where('user_id', Auth::user()->id)->where('roomID', $DC->first()->id)->orderby('bot_id')->get() as $chat)
                            <div
                                ...
                                <div class="h-full w-full"><img data-tooltip-target="{{$extra}}llm_{{$chat->id}}_{{ $chat->bot_id }}"
            ...
            $join->on('llms.id', '=', 'bots.model_id');
                        ...
                        <div
                            class="mx-1 flex-shrink-0 h-5 w-5 rounded-full border border-gray-400 dark:border-gray-900 bg-black flex items-center justify-center overflow-hidden">
                            <div class="h-full w-full"><img data-tooltip-target="{{$extra}}llm_{{ $chat->bot_id }}"
                                    data-tooltip-placement="top" class="h-full w-full"
