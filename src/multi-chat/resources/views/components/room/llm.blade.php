            ...
            <div class="mb-2 rounded-lg">
                ...
                @if (request()->user()->hasPerm('Room_update_new_chat'))
                    <form method="post" action="{{ route('room.new') }}">
                        @csrf
                        <button
                            class="flex items-center px-2 scrollbar rounded-t-lg w-full hover:bg-gray-300 dark:hover:bg-gray-700 scrollbar-3 overflow-x-auto py-2 border-b border-black dark:border-white">
                ...
                @else
                    <div
                        class="flex px-2 scrollbar rounded-t-lg w-full scrollbar-3 overflow-x-auto py-2 border-b border-black dark:border-white">
                        @include('components.room.chat_button', ['chats' => $chats, 'extra' => $extra])
                        @if (count($chats) == 1)
                            <span
                                class="text-center w-full line-clamp-1 text-black dark:text-white">{{ $chats[0]->name }}</span>
