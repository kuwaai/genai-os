...
<div>
    <h2 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">{{ __('storage.label.running_download_jobs') }}
    </h2>
    <ul id="jobs-list"
    ...
    function fetchJobsData() {
        $.getJSON('{{ route('manage.kernel.storage.jobs') }}', function(data) {
            ...
            if (data.active_jobs && data.active_jobs.length > 0) {
                ...
                $('#jobs-list').append(
                    '<li class="text-gray-500 dark:text-gray-400">{{ __('storage.label.no_active_jobs') }}</li>'
                    );
            }
        }).fail(function() {
            $('#jobs-list').empty(); // Clear previous data
            $('#jobs-list').append(
                '<li class="text-red-500 dark:text-red-400">{{ __('storage.label.error_fetch_jobs_data') }}</li>'
                );
        });
    }
