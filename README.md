...
3. **Configure PHP and PHP-FPM:**
   ...
   - Configure your web server (Nginx or Apache) to serve the project's files. Set `src/multi-chat/public` as the html root.
   - Example setting file: `src/multi-chat/nginx_config_example`, `src/multi-chat/php.ini`
...
4. **Configure Redis:**
   ...
   - Start Redis worker by running `php artisan queue:work --timeout=0` under `src/multi-chat/` to process requests to the agent, It's recommanded to have at least 5 workers running at the same time.

5. **Run the application:**
   - Start your web server and PHP-FPM.
   - Run the agent in `src/kernel/main.py` with your Python installation. It's recommended to copy the agent folder to another location before execution.

6. **Access the application:**
   - First you need to seed a account, go `src/multi-chat/` and run `php artisan db:seed --class=AdminSeeder --force` to seed your first admin account.
   - Open your web browser and go to the application's URL.
   - Login with your admin account and start using Kuwa GenAI OS

7. **Setting up Models:**
    - By default, there are no models installed. Please read [this README](./src/executor/README.md) to set up some models.
