                        ...
                        <div class="mb-2 border border-black dark:border-white border-1 rounded-lg">
                            ...
                            @foreach (App\Models\Chats::where('user_id', Auth::user()->id)->where('llm_id', $LLM->id)->orderby('name')->get() as $chat)
                                <div
                                    ...
                                    <a class="flex menu-btn flex text-gray-700 dark:text-white w-full h-12 dark:hover:bg-gray-700 hover:bg-gray-200 {{ request()->route('chat_id') == $chat->id ? 'bg-gray-200 dark:bg-gray-700' : '' }} transition duration-300"
                                        href="{{ route('chats', $chat->id) }}">
                                        <p class="flex-1 flex items-center justify-center text-center overflow-y-auto leading-none">{{ $chat->name }}</p>
                                    </a>
                                </div>
                            @endforeach
                        </div>
